...
Assume each case
Multilevel analysis -- each case behave differently
...
df <- included_models_summary %>%
  full_join(DRIInd.LLMs) %>%
  select(model, series, version, case, DRIPostV3) %>%
  arrange(version)
...
llm_scores_reason <- llm_scores %>%
  ...
  mutate(
    is_reasoner = case_when(reasoning == TRUE ~ TRUE, .default = FALSE)
  )
