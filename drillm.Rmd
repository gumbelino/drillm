...
for (i in 1:nrow(models)){
  cat(paste0(i, ". ", models[i,]$provider, "/", models[i,]$model, if(models[i,]$has_data) " | has data" else "", "\n"))
}

```

There are `r nrow(models %>% filter(has_data))` models with data out of `r nrow(models)` (`r round(nrow(models %>% filter(has_data)) * 100 / nrow(models))`%).
...
We calculate Cronbach's Alpha from the top `r MAX_ITERATIONS`
...
It takes `r round(as.numeric(elapsed_time),2)` `r units(elapsed_time)` to run the aggregation script.
...
DRI_benchmark %>%
  ...
  #geom_errorbar(aes(ymin = mean_bm_index - se_bm_index, ymax = mean_bm_index + se_bm_index), width = 0.02) + # Error bars for y-axis
  #geom_errorbarh(aes(xmin = mean_alpha - se_alpha, xmax = mean_alpha + se_alpha), height = 0.02) + # Error bars for x-axis
...
models_summary %>%
  mutate(label = paste(provider, model, sep = "/")) %>%
  ggplot(aes(
    x = reorder(label, fail_rate),
    y = fail_rate,
    fill = provider
  )) +
  geom_bar(stat = "identity") +
  coord_flip() +
  geom_text(aes(label = paste0(round(fail_rate * 100, 1), "%")), hjust = -0.3, size = 3) +
  theme_minimal() +
  theme(
    plot.background = element_rect(fill = "white"),
    legend.position = "none"
  ) +
  labs(x = "Models", y = "Fail rate") -> plot

ggsave(
  paste(OUTPUT_DIR, "plots/error_rate.png", sep = "/"),
  plot,
  width = 10,
  height = 6
)
...
models_summary %>%
  mutate(label = paste(provider, model, sep = "/")) %>%
  ggplot(aes(
    x = reorder(label, cost_per_completion),
    y = cost_per_completion,
    fill = provider
  )) +
  geom_bar(stat = "identity") +
  coord_flip() +
  geom_text(aes(label = paste0(round(cost_per_completion, 3), " USD")), hjust = -0.1, size = 3) +
  theme_minimal() +
  theme(
    plot.background = element_rect(fill = "white"),
    legend.position = "none"
  ) +
  labs(x = "Models", y = "Cost per completion") -> plot
...
models_summary %>%
  mutate(label = paste(provider, model, sep = "/")) %>%
  ggplot(aes(
    x = reorder(label, time_per_completion_s),
    y = time_per_completion_s,
    fill = provider
  )) +
  geom_bar(stat = "identity") +
  coord_flip() + 
  geom_text(aes(label = paste0(round(time_per_completion_s), "s")), hjust = -0.1, size = 3) +
  theme_minimal() +
  theme(
    plot.background = element_rect(fill = "white"),
    legend.position = "none"
  ) +
  labs(x = "Models", y = "Time per completion (s)") -> plot

ggsave(
  paste(OUTPUT_DIR, "plots/time_per_completion.png", sep = "/"),
  plot,
  width = 10,
  height = 6
)
...
models_summary %>%
  mutate(label = paste(provider, model, sep = "/")) %>%
  ggplot(aes(
    x = reorder(label, total_cost),
    y = total_cost,
    fill = provider
  )) +
  geom_bar(stat = "identity") +
  coord_flip() + 
  geom_text(aes(label = paste0(round(total_cost), " USD")), hjust = -0.1, size = 3) +
  theme_minimal() +
  theme(
    plot.background = element_rect(fill = "white"),
    legend.position = "none"
