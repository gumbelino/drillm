...
LLM_DATA_DIR <- "llm_data" #"llm_data_part_1_2025-08-12"
...
models <- models %>% filter(included) %>% filter(is.na(context_length))
...
get_summary_stats(alpha_results, type="robust")
...
# overwrite llm_data with right swiss cases
write_csv(llm_data, paste(OUTPUT_DIR, "llm_data.csv", sep = "/"))
...
included_cases <- cases %>% filter(is.na(included))
...
# Thawil does not align with experience with deliberative democracy
...
write_output_csv(human_data, "human_data.csv")
...
get_case_DRI <- function(IC, type="human_only", model=NA, it = NA) {
...
for (case_study in 1:nrow(mini_publics)) {
  ...
  for (llm in 1:nrow(llms)) {
    ...
    for (it in 1:nrow(llm_survey_data)) {
      ...
      DRI.Case.LLM <- get_case_DRI(IC.LLM, type, model, it)
      ...
      #DRI.Group.LLM <- rbind(DRI.Group.LLM, DRI.Case.LLM)
...
DRI.Global <- DRI.Global %>%
  mutate(
    ...
    DRI_DELTA_V3 = DRI_POST_V3 - DRI_PRE_V3,
  )
...
df %>%
  ...
  geom_point(
    data = . %>% filter(signif),
    aes(x = -0.5, y = 5), color = "red", shape = 8, size = 5)
...
```{r H1-posthoc, fig.width=10,fig.height=6}
...
if (kruskal_test_result$p.value < 0.05) {
  ...
  random_comp <- dunn_test_result %>%
    ...
    mutate(P.rounded = round(P.adjusted, 5)) %>%
  ...
  random_comp <- random_comp %>%
    ...
    mutate(label = if_else(P.adjusted <= 0.0001, paste0(label, "*"), label)) %>%
    mutate(label = if_else(P.adjusted <= 0.001, paste0(label, "*"), label)) %>%
    mutate(label = if_else(P.adjusted <= 0.01, paste0(label, "*"), label)) %>%
    mutate(label = if_else(P.adjusted <= 0.05, paste0(label, "*"), label))
...
df %>%
    full_join(random_comp, by=join_by(model)) %>%
  mutate(label = if_else(is.na(label), source, label)) %>%
ggplot(aes(x = reorder(label, DRIPostV3, FUN = median), y = DRIPostV3, fill = source_type)) +
  geom_boxplot() +
  labs(title = "LLM (Individual) vs. Random (Individual) DRI",
       x = "Source",
       y = "DRI (Post-Deliberation)") +
  theme_minimal() + 
  theme(
    plot.background = element_rect(fill = "white"),
    legend.position = "bottom right",
...
ggsave(
  paste(OUTPUT_DIR, "plots", "H1-random.png", sep = "/"),
  plot,
  width = 10,
  height = 6
)
...
df %>%
  ggplot(aes(
    x = reorder(source, DRIPostV3, FUN = median),
    y = DRIPostV3,
    fill = source_type
  )) +
  geom_boxplot() +
  labs(title = "Distribution of DRIPostV3 for Each Source Type", x = "Source", y = "DRIPostV3") +
  theme_minimal() +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 70, hjust = 1))


p <- ggboxplot(df, x = "source", y = "DRIPostV3",
          color = "source_type") +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 70, hjust = 1))

#  Add p-value#  Add p-valuemedian()
p + stat_compare_means(label.x = "command") +
  stat_compare_means(label = "p.signif", method = "t.test", ref.group = "human_only_mean", hide.ns = TRUE, vjust = 1)
...
```{r H2-posthoc, fig.width=10,fig.height=6}
...
if (kruskal_test_result$p.value < 0.05) {
  ...
  human_comp <- dunn_test_result %>%
    ...
    mutate(P.formated = ifelse(P.adjusted < 0.05, paste0(P.adjusted, "*"), P.adjusted))
  
  # Print the results of Dunn's test with Bonferroni correction
  human_comp <- human_comp %>%
    arrange(P.adjusted) %>%
    mutate(model = str_remove(comparisons, " - human_only_mean")) %>%
    mutate(model = str_remove(model, "human_only_mean - "))
...
# Assuming your dataframe is named `df`
df %>%
  full_join(human_comp, by=join_by(model)) %>%
  mutate(label = if_else(is.na(label), source, label)) %>%
  ggplot(aes(
    x = reorder(label, DRIPostV3, FUN = median),
    y = DRIPostV3,
    fill = source_type
  )) +
  geom_boxplot() +
  labs(title = "LLM (Individual) vs. Human (Mean) DRI", x = "Source", y = "DRI (Post-Deliberation)") +
  theme_minimal() +
  theme(
    plot.background = element_rect(fill = "white"),
    legend.position = "bottom right",
...
ggsave(
  paste(OUTPUT_DIR, "plots", "H2-human.png", sep = "/"),
  plot,
  width = 10,
  height = 6
)
...
res.kruskal <- DRIInd.ALL %>% kruskal_test(DRIPostV3 ~ source)
res.kruskal
...
pwc1 <- pwc %>%
  filter(group1 == "human_only_mean" | group2 == "human_only_mean") %>%
  mutate(
    model = ifelse(group1 == "human_only_mean", group2, group1),
    ref = "human_only_mean"
  ) %>%
  filter(p.adj.signif != "ns") %>%
  select(model, p.adj.signif, ref)

#pwc1[pwc1$model == "gpt-4", ]$p.adj.signif

pwc2 <- pwc %>%
  filter(group1 == "random" | group2 == "random") %>%
  mutate(
    model = ifelse(group1 == "random", group2, group1),
    ref = "random"
  ) %>%
  filter(p.adj.signif != "ns") %>%
  select(model, p.adj.signif, ref)
...
ggboxplot(x = "source", y = "DRIPostV3", color = "source_type", order = order) +
  # stat_pvalue_manual(pwc1, hide.ns = TRUE, y.position = 1, color = "red", step.increase = 0.06) +
  # stat_pvalue_manual(pwc2, hide.ns = TRUE, y.position = 1, color = "blue", step.increase = 0.06) +
  rotate_x_text(70) +
  scale_x_discrete(
    labels = ~ case_when(
      .x == "random" ~ paste0("<span style='color:#619CFF'>", .x, "</span>"),
      .x == "human_only_mean" ~ paste0("<span style='color:#e76c68'>", .x, "</span>"),
      .x %in% pwc2$model ~ paste0(.x, " <b style='color:#619CFF'>", "*", "</b>"),
      .x %in% pwc1$model ~ paste0(.x, " <b style='color:#e76c68'>", "*", "</b>"),
      .default = .x
    )
  ) +
  labs(
    title = "LLM (Individual) vs. Human (Mean) vs. Random (Individual) DRI",
...
ggsave(
  paste(OUTPUT_DIR, "plots", "H1H2.png", sep = "/"),
  plot,
  width = 10,
  height = 8
)

pr <- included_models_summary %>%
...
for (series in v_series) {
  ...
  series_df <- df %>% filter(series == !!series)
  num_v <- length(unique(series_df$version))
  
  if (num_v < 2) {
    next
  }
  
  #cat(series, num_v, "versions\n")
  
  series_df <- series_df %>%
    mutate(id = as.factor(paste(series, case, sep = "/"))) %>%
    select(series, id, version, DRIPostV3)

  v_vals <- sort(unique(series_df$version))
  ...
  if (nrow(plot_data) == 0) {
    ...
    plot_data <- bind_rows(plot_data, series_df)
  }
  ...
  ggqqplot(series_df, "DRIPostV3", facet.by = "v")
  
  res.fried <- series_df %>% friedman_test(DRIPostV3 ~ v | id)
  res.fried
...
# plot data
plot_data %>%
  full_join(res, by=join_by(series)) %>%
  mutate(label = paste0("p=", round(p, 3))) %>%
  ggplot(aes(x=v, y=DRIPostV3)) + 
  geom_violin() +
...
qual_data <- llm_data %>% 
  select(provider, model, survey, R) %>%
  group_by(provider, model, survey) %>%
  slice_head(n=1)
