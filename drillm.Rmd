...
models %>%
  select(provider, model, series, num_parameters, context_length, arch, version) %>% 
  arrange(provider, model) %>%
  knitr::kable(caption = "LLMs", row.names = TRUE, col.names = c("Provider", "Model", "Series", "Parameters (B)", "Context Length", "Architecture", "Version"))
...
DRIInd.ALL <- DRIInd.LLMs %>%
  mutate(source = model) %>% 
  select(source, survey, case, DRIPreV3, DRIPostV3)

DRIInd.ALL <- DRIInd.Rands %>% mutate(source = "random") %>%
  select(source, survey, case, DRIPreV3, DRIPostV3) %>%
...
# print(shapiro_results %>% filter(shapiro_p > 0.05))
...
# plot distributions
ggplot(df, aes(x = DRIPostV3, fill = source)) +
...
if (kruskal_test_result$p.value < 0.05) {
  ...
  random_comp %>% arrange(P.adjusted) %>% select(comparisons, P.formated) %>%
    knitr::kable(
      caption = "",
      col.names = c("Comparisons", "P-adjusted"),
      digits = 3
    )
...
llm_scores <- llm_scores %>%
  mutate(
    ...
    architecture_type = case_when(
      grepl("command", model, ignore.case = TRUE) ~ "Instruction-tuned Dense Transformer",
      grepl("gpt-4o|gpt-4o-mini", model, ignore.case = TRUE) ~ "Multimodal Dense Transformer",
      grepl("gpt-4|gpt-3.5", model, ignore.case = TRUE) ~ "Dense Transformer",
      grepl("claude-3", model, ignore.case = TRUE) ~ "Dense Transformer",
      grepl("gemini|gemini-1.5|gemini-2.0", model, ignore.case = TRUE) ~ "Multimodal Hybrid",
      grepl("mixtral|open-mixtral", model, ignore.case = TRUE) ~ "Mixture of Experts (MoE)",
      grepl("mistral-large|mistral-small|open-mistral|ministral", model, ignore.case = TRUE) ~ "Dense Transformer",
      grepl("gemma", model, ignore.case = TRUE) ~ "Dense Transformer",
      grepl("llama3|llama", model, ignore.case = TRUE) ~ "Dense Transformer",
      grepl("phi", model, ignore.case = TRUE) ~ "Dense Transformer",
      grepl("qwen1.5|qwen", model, ignore.case = TRUE) ~ "Dense Transformer",
      grepl("qwen2", model, ignore.case = TRUE) ~ "Mixture of Experts (MoE)",
      grepl("deepseek-v2", model, ignore.case = TRUE) ~ "Mixture of Experts (MoE)",
      grepl("deepseek", model, ignore.case = TRUE) ~ "Dense Transformer",
      grepl("o1", model, ignore.case = TRUE) ~ "Dense Transformer",  # o1 known, reasoner
      grepl("o3", model, ignore.case = TRUE) ~ "Unknown",          # Speculative
      grepl("qwq", model, ignore.case = TRUE) ~ "Unknown",         # Typo or placeholder
      TRUE ~ "Unknown"
    ),
    
    # Neural design
    neural_architecture = case_when(
      grepl("mixtral|open-mixtral|qwen2|deepseek-v2", model, ignore.case = TRUE) ~ "Sparse MoE Decoder",
      grepl("gpt-4o|gpt-4o-mini|gemini|gemini-1.5|gemini-2.0", model, ignore.case = TRUE) ~ "Multimodal Decoder",
      grepl("gpt|claude|command|llama|phi|mistral|gemma|qwen|deepseek|o1", model, ignore.case = TRUE) ~ "Dense Decoder-only Transformer",
      grepl("o3|qwq", model, ignore.case = TRUE) ~ "Other/Unknown",
      TRUE ~ "Other/Unknown"
    ),
    
    # Reasoner classification
    is_reasoner = case_when(
      grepl("o1", model, ignore.case = TRUE) ~ TRUE,           # OpenAI o1: Explicit reasoner
      grepl("claude-3", model, ignore.case = TRUE) ~ TRUE,     # Claude 3: Strong reasoner
      grepl("gpt-4o|gpt-4", model, ignore.case = TRUE) ~ TRUE, # GPT-4 variants: Advanced reasoning
      grepl("mistral-large", model, ignore.case = TRUE) ~ TRUE,# Mistral Large: Some reasoning
      TRUE ~ FALSE                                             # Others: General-purpose
    )
  )
...
ggplot(llm_scores, aes(x = is_reasoner, y = aggregate_index, fill = is_reasoner)) +
