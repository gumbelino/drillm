...
labels <- grouped_data[which(grouped_data$alpha > 0.85), ]$model
...
grouped_data %>%
  ggscatter(
    ...
    xlab = "Post-Deliberation DRI (Average Across Model Iterations)",
...
res.wilcox <- grouped_data %>%
  as.data.frame() %>%
  wilcox_test(alpha ~ is_reasoner_clean)
res.wilcox
...
grouped_data %>%
  ...
  stat_summary(
    fun.data = function(x)
      data.frame(y = 0.9, label = paste0("\U00B5 = ", round(mean(
        x
      ), 2))),
    geom = "text"
  ) +
...
res.wilcox <- grouped_data %>%
  as.data.frame() %>%
  wilcox_test(DRI ~ is_reasoner_clean)
res.wilcox
...
grouped_data %>%
  ...
  stat_summary(
    fun.data = function(x)
      data.frame(y = 0.7, label = paste0("\U00B5 = ", round(mean(
        x
      ), 2))),
    geom = "text"
  ) +
...
selected_data <- grouped_data %>%
  filter(model %like% "grok-3-mini" | model %like% "claude-3-7-sonnet-20250219")
